...
for t=2:total_steps
    ...
    scale_factor = runge_step(scale_factor,t,area,step,expand_or_contract,simType); %runge-kutta algorithm
    
    %DERIVATIVES
    if(t>=2)
        scale_1deriv(1,t) = slope_btwn(scale_factor,t,t-1);
    end
    if(t>=3)
        scale_2deriv(1,t) = slope_btwn(scale_1deriv,t,t-1);
    end
    if(t>=4)
        scale_3deriv(1,t) = slope_btwn(scale_2deriv,t,t-1);
    end
    ...
    if t>=4 %only use coefficients once all derivatives have real values
