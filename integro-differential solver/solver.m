...
t0_minus_3 = t0-3*step;

%setting up functions filled with NaNs, and t values, spaced by the step. 
%NaNs are first row, t values are second row. 
[scale_factor,scale_1deriv,scale_2deriv,scale_3deriv,r_func1,r_func2,r_func3,basem_scale_factor,basem_scale_1deriv] = deal(NaN(2,total_steps+3));
...
for curr_t_index=1:total_steps
    ...
    if(curr_t_index>=3)
        scale_1deriv(1,curr_t_index-1) = slope_btwn(scale_factor,curr_t_index,curr_t_index-2);
    end
    if(curr_t_index>=5)
        scale_2deriv(1,curr_t_index-2) = slope_btwn(scale_1deriv,curr_t_index-1,curr_t_index-3);
    end
    if(curr_t_index>=7)
        scale_3deriv(1,curr_t_index-3) = slope_btwn(scale_2deriv,curr_t_index-2,curr_t_index-4);
    end
