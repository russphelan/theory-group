...
[scale_factor,scale_1deriv,scale_2deriv,scale_3deriv,r_func1,r_func2,r_func3,basem_scale_factor,basem_scale_1deriv,basem_scale_2deriv,basem_scale_3deriv,basem_2nd_scale_1deriv,basem_2nd_scale_factor] = deal(NaN(2,total_steps));
for i=1:total_steps
    [scale_factor(2,i),scale_1deriv(2,i),scale_2deriv(2,i),scale_3deriv(2,i),basem_scale_factor(2,i),basem_scale_1deriv(2,i),basem_scale_2deriv(2,i),basem_scale_3deriv(2,i),basem_2nd_scale_1deriv(2,i),basem_2nd_scale_factor(2,i),r_func1(2,i),r_func2(2,i),r_func3(2,i)] = deal(t0 + (i-1)*step);
end

scale_factor(1,1) = a0; %initial conditions
basem_scale_factor(1,1) = a0; %initial conditions
basem_2nd_scale_factor(1,1) = a0;
area_matrix = [0 0 0];
basem_area_matrix = [0 0 0];
...
for t=1:total_steps
    ...
    if(t>=4)
        ...
        if(t>=3)
            basem_scale_2deriv(1,t) = slope_btwn(basem_scale_1deriv,t,t-1);
        end
        if(t>=4)
            basem_scale_3deriv(1,t) = slope_btwn(basem_scale_2deriv,t,t-1);
        end
    else
...
%setting up functions filled with NaNs, and t values, spaced by the step. 
%NaNs are first row, t values are second row. 
[r_func1,r_func2,r_func3] = deal(NaN(2,total_steps));
for i=1:total_steps
    [r_func1(2,i),r_func2(2,i),r_func3(2,i)] = deal(t0 + (i-1)*step);
end

for t=1:8676 %total_steps
    
    %calculate area up to current t value
    if(t>=4)
        %r functions
        %display(t)
        r_func1 = r_funcs(r_func1,t,basem_scale_factor,basem_scale_1deriv,basem_scale_2deriv,basem_scale_3deriv,1);
        r_func2 = r_funcs(r_func2,t,basem_scale_factor,basem_scale_1deriv,basem_scale_2deriv,basem_scale_3deriv,2);
        r_func3 = r_funcs(r_func3,t,basem_scale_factor,basem_scale_1deriv,basem_scale_2deriv,basem_scale_3deriv,3);
        ...
        %set coefficients for combining areas
        coef1 = 6*sqrt(basem_scale_factor(1,t))*basem_scale_2deriv(1,t);
        coef2 = 6*basem_scale_1deriv(1,t)^2/sqrt(basem_scale_factor(1,t));
        coef3 = 12*sqrt(basem_scale_factor(1,t))*basem_scale_1deriv(1,t);
        
        %the actual combining of areas
        area = N*(coef1*area1 + coef2*area2 + coef3*area3);
        
        %storing area for plotting
        basem_area_matrix = [basem_area_matrix area];
        
        %calculate next runge_step using the area just calculated
        basem_2nd_scale_factor = runge_step(basem_2nd_scale_factor,t,area,step,expand_or_contract,2); %runge-kutta algorithm
        if t>=325
            basem_2nd_scale_1deriv(1,t) = slope_btwn(basem_2nd_scale_factor,t,t-1);
        end
    else
        basem_2nd_scale_factor = runge_step(basem_2nd_scale_factor,t,0,step,expand_or_contract,2); %runge-kutta algorithm
        if t>=325
            basem_2nd_scale_1deriv(1,t) = slope_btwn(basem_2nd_scale_factor,t,t-1);
        end
    end
...
%basem scale factor second derivative plot
subplot(2,3,2);
plot(basem_scale_2deriv(2,1:total_steps),basem_scale_2deriv(1,1:total_steps),'LineWidth',lw);
xlabel('Time (s)','FontSize',14,'interpreter','latex');
ylabel('$a(t)$','FontSize',14,'interpreter','latex');
title('Basem Scale Factor','FontSize',18,'FontWeight','bold','interpreter','latex');

%basem scale factor third derivative plot
subplot(2,3,3);
plot(basem_scale_3deriv(2,1:total_steps),basem_scale_3deriv(1,1:total_steps),'LineWidth',lw);
xlabel('Time (s)','FontSize',14,'interpreter','latex');
ylabel('$a(t)$','FontSize',14,'interpreter','latex');
title('Basem Scale Factor','FontSize',18,'FontWeight','bold','interpreter','latex');


%basem 2nd scale factor plot
% subplot(2,3,4);
% plot(basem_2nd_scale_factor(2,1:total_steps),basem_2nd_scale_factor(1,1:total_steps),'LineWidth',lw);
% xlabel('Time (s)','FontSize',14,'interpreter','latex');
% ylabel('$a(t)$','FontSize',14,'interpreter','latex');
% title('Basem 2nd Scale Factor','FontSize',18,'FontWeight','bold','interpreter','latex');

%basem 2nd scale factor derivative plot
subplot(2,3,4);
plot(basem_2nd_scale_1deriv(2,1:total_steps),basem_2nd_scale_1deriv(1,1:total_steps),'LineWidth',lw);
xlabel('Time (s)','FontSize',14,'interpreter','latex');
ylabel('$a(t)$','FontSize',14,'interpreter','latex');
title('Basem 2nd Scale Factor Derivative','FontSize',18,'FontWeight','bold','interpreter','latex');

%area plot
subplot(2,3,5);
plot(scale_2deriv(2,1:length(basem_area_matrix)),basem_area_matrix(1,:),'LineWidth',lw,'Color','r');
xlabel('Time (s)','FontSize',14,'interpreter','latex');
ylabel('Area from $t_0$','FontSize',14,'interpreter','latex');
title('Area','FontSize',18,'FontWeight','bold','interpreter','latex');
